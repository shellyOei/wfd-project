...
@section('page-title', 'Add New Doctor Availability')

@section('content')
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 max-w-2xl mx-auto">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Availability Details</h3>
        ...
        <form action="{{ route('admin.day-availables.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="time" name="start_time" id="start_time" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value="{{ old('start_time') }}" required>
                </div>
                <div>
                    <label for="end_time" class="block text-sm font-medium text-gray-700">End Time <span class="text-red-500">*</span></label>
                    <input type="time" name="end_time" id="end_time" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value="{{ old('end_time') }}" required>
                </div>
            </div>
