...
@section('content')
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Edit Doctor Record</h3>
        ...
        <form action="{{ route('admin.doctors.update', $doctor->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="doctor_number" id="doctor_number" value="{{ old('doctor_number', $doctor->doctor_number) }}" required
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="front_title" id="front_title" value="{{ old('front_title', $doctor->front_title) }}"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    @error('front_title')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="back_title" id="back_title" value="{{ old('back_title', $doctor->back_title) }}"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    @error('back_title')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <select name="specialization_id" id="specialization_id" required
                        ...
                        @foreach ($specializations as $spec)
                            <option value="{{ $spec->id }}" {{ (old('specialization_id', $doctor->specialization_id) == $spec->id) ? 'selected' : '' }}>
                                {{ $spec->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mb-6">
                ...
                <textarea name="address" id="address" rows="3"
                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('address', $doctor->address) }}</textarea>
                @error('address')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="photo" id="photo" value="{{ old('photo', $doctor->photo) }}"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                @error('photo')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="5"
                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description', $doctor->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
